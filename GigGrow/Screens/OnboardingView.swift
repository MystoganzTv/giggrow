...
struct OnboardingView: View {
    ...
    private var progress: some View {
        HStack(spacing: 6) {
            ForEach(0..<stepCount, id: \.self) { index in
                Capsule()
                    .fill(index <= step ? GG.Palette.violet400 : Color.white.opacity(0.12))
                    .frame(height: 3)
            }
        }
        .padding(.horizontal, GG.Layout.screenInset)
    ...
    private var vehicleStep: some View {
        VStack(alignment: .leading, spacing: GG.Layout.stackSpacing) {
            stepHeader(
                ...
                subtitle: "GigGrow tracks the car as part of the business — what it costs, and what to set aside for repairs."
            )
    ...
    private var platformStep: some View {
        VStack(alignment: .leading, spacing: GG.Layout.stackSpacing) {
            stepHeader(
                ...
                subtitle: "Pick every one you use. Running two at once is normal — GigGrow is built for it."
            )
            ...
            Text(selectedPlatforms.isEmpty
                ...
                .ggText(GG.Typo.footnote,
                        color: selectedPlatforms.isEmpty ? GG.Palette.amber.opacity(0.85)
                                                         : GG.Ink.muted)
                .padding(.leading, 6)
        }
    }
    ...
    private func platformRow(_ account: PlatformAccount) -> some View {
        ...
        return Button {
            ...
            HStack(spacing: 13) {
                ...
                Text(account.name)
                    .ggText(GG.Typo.rowLabel, color: isOn ? GG.Ink.primary : GG.Ink.tertiary)
                ...
                ZStack {
                    ...
                    if isOn {
                        Circle().fill(GG.Palette.violet500)
                        CheckGlyph()
    ...
    private func field(_ label: String, text: Binding<String>,
        ...
        HStack {
            ...
            if optional {
                Text("optional")
                    .ggText(.system(size: 11, weight: .medium), color: GG.Ink.faint)
            }
            Spacer(minLength: 12)
    ...
    private var footer: some View {
        VStack(spacing: 12) {
            Button(action: advance) {
                Text(step == stepCount - 1 ? "Start tracking" : "Continue")
                    ...
                    .padding(.vertical, 16)
                    .background(
                        canAdvance ? AnyShapeStyle(GG.Gradients.brandMark)
        ...
        .padding(.top, 12)
        .padding(.bottom, 20)
        .background(GG.Palette.screen.opacity(0.9))
    }
