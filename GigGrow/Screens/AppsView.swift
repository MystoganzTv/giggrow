...
struct AppsView: View {
    ...
    private var header: some View {
        HStack(alignment: .firstTextBaseline) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(subtitle)
                    .ggText(GG.Typo.subtitle, color: Color.white.opacity(0.45))
            }
    ...
    private var combinedCard: some View {
        HeroCard(radius: GG.Radius.card,
                 ...
                 gradient: GG.Gradients.heroBlue) {
            ...
            HStack(spacing: 14) {
                Text(platform.initial)
                    ...
                    .background(
                        ...
                        in: RoundedRectangle(cornerRadius: GG.Radius.badge, style: .continuous)
                    )
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text(platform.meta)
                        .ggText(.system(size: 13, weight: .regular), color: GG.Ink.tertiary)
                }
                ...
                VStack(alignment: .trailing, spacing: 4) {
                    Text(Money.cents(snapshot.amount(for: platform)))
                        .ggText(.system(size: 17, weight: .semibold), tracking: -0.34)
                    Text(platform.delta)
                        .ggText(.system(size: 11.5, weight: .semibold), color: GG.Palette.mint)
                }
            }
        }
