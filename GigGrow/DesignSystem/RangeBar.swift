                ...
                Button {
                    ...
                    HStack(spacing: 5) {
                        Text(selection.title)
                            .ggText(.system(size: 15, weight: .semibold), tracking: -0.2)
    ...
    private func arrow(_ delta: Int, systemName: String, enabled: Bool) -> some View {
        Button {
            ...
            Image(systemName: systemName)
                ...
                .foregroundStyle(enabled ? GG.Ink.secondary : GG.Ink.faint)
                .frame(width: 38, height: 38)
                .background(enabled ? GG.Surface.glassFaint : Color.clear, in: Circle())
        }
        .buttonStyle(.plain)
        .disabled(!enabled)
    }
    ...
    struct Harness: View {
        ...
        var body: some View {
            VStack(spacing: 30) {
                ...
                Text(selection.title).ggText(GG.Typo.footnote, color: GG.Ink.muted)
            }
            .padding()
            .background(GG.Palette.screen)
        }
    }
