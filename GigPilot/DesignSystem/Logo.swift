...
//  GigPilot
//
//  Vector rebuild of the GigPilot brand mark: a gauge that closes into a map
...
/// The GigPilot mark. Set `glow` for the neon bloom used on dark surfaces.
struct GigPilotLogo: View {
    var size: CGFloat = 44
    var glow: Bool = true
...
struct GigPilotIconTile: View {
    ...
    var body: some View {
        ZStack {
            ...
            GigPilotLogo(size: size * 0.68, glow: false)
        }
...
#Preview("Brand mark") {
    ZStack {
        ...
        VStack(spacing: 40) {
            GigPilotLogo(size: 160)
            HStack(spacing: 24) {
                GigPilotLogo(size: 44)
                GigPilotLogo(size: 28)
                GigPilotIconTile(size: 60)
            }
        }
    }
}
