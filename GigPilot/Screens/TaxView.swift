...
//  only place in GigPilot where a figure might end up on a return.
//
...
struct TaxView: View {
    ...
    private var workingOut: some View {
        GlassCard {
            VStack(spacing: 0) {
                ...
                RowDivider(color: GP.Surface.divider)
    ...
    private var breakdown: some View {
        GlassCard {
            VStack(spacing: 0) {
                ...
                RowDivider(color: GP.Surface.divider)
                line("Left after tax", estimate.netAfterTax, emphasised: true, tint: GP.Palette.mint)
            }
        }
    }
    ...
    private func line(_ label: String, _ amount: Double,
        ...
        HStack {
            Text(label)
                .gpText(emphasised ? .system(size: 15.5, weight: .semibold) : GP.Typo.rowLabel)
            Spacer(minLength: 12)
            Text(Money.cents(amount))
                .gpText(.system(size: emphasised ? 16 : 15, weight: .semibold), tracking: -0.3,
                        color: tint ?? GP.Ink.primary)
        }
        .padding(.vertical, 13)
    }
    ...
    private var midYear: Date {
        Calendar.gigPilot.date(from: DateComponents(year: year, month: 7, day: 1)) ?? .now
    }
}
