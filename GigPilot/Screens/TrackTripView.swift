...
struct TrackTripView: View {
            ...
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    ...
                    Button(isRecording ? "Cancel trip" : "Cancel") { cancel() }
                        .foregroundStyle(isRecording ? GP.Palette.amber : GP.Ink.secondary)
                }
            }
        }
    ...
    private var controlCard: some View {
        GlassCard(radius: GP.Radius.hero,
                  padding: EdgeInsets(top: 22, leading: 20, bottom: 22, trailing: 20)) {
            switch tracker?.status {
            case .denied:
                blocked("Location is off for GigPilot. Turn it on in iOS Settings and this will work.")
            case .restricted:
    ...
    private var idle: some View {
        HStack(alignment: .center, spacing: 16) {
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text("GigPilot follows you from A to B and works out the distance. Keep the app open, or allow location “Always” to leave it running in your pocket.")
    ...
    private var running: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack(spacing: 9) {
                PulsingDot()
                Text("Recording")
                    .gpText(.system(size: 12.5, weight: .semibold), tracking: 0.6,
                            color: GP.Palette.mint)
                Spacer(minLength: 0)
                Text(elapsed)
                    .gpText(.system(size: 15, weight: .medium).monospacedDigit(),
            ...
            HStack(alignment: .firstTextBaseline, spacing: 6) {
                Text(String(format: "%.1f", tracker?.currentMiles ?? 0))
                    ...
                    .monospacedDigit()
                Text("mi")
                    .gpText(.system(size: 20, weight: .medium), color: GP.Ink.tertiary)
            }
...
private struct PulsingDot: View {
    ...
    var body: some View {
        Circle()
            ...
            .overlay(
                Circle()
                    .stroke(GP.Palette.mint.opacity(isExpanded ? 0 : 0.6), lineWidth: 2)
