...
struct MileageView: View {
    ...
    @State private var year: Int = Calendar.gigPilot.component(.year, from: .now)
        ...
        VStack(alignment: .leading, spacing: 5) {
            ...
            HStack(spacing: 6) {
                ...
                Text(Money.cents(yearDeduction))
                    .gpText(.system(size: 14.5, weight: .semibold), color: GP.Palette.mint)
            }
        ...
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                Text(autoTrackingLabel)
                    .gpText(.system(size: 14.5, weight: .semibold), color: autoTrackingColour)
    ...
    private var autoTrackingColour: Color {
        switch tracker?.status {
        case .recording, .waiting:   return GP.Palette.mint
        case .denied, .restricted:   return GP.Palette.amber
        default:                     return GP.Ink.tertiary
        }
    }
        ...
        HStack {
            Button { isPickingYear = true } label: {
                Text("Filter: \(String(year))")
                    .gpText(.system(size: 14, weight: .semibold), color: GP.Palette.violet300)
            }
            .buttonStyle(.plain)
            ...
            if let undo {
                Button {
                    ...
                    Text("Undo")
                        .gpText(.system(size: 14, weight: .semibold), color: GP.Ink.secondary)
                }
                .buttonStyle(.plain)
            } else if !visible.isEmpty {
                Text("\(visible.count) drive\(visible.count == 1 ? "" : "s") · \(String(format: "%.0f mi", visibleMiles))")
                    .gpText(GP.Typo.captionMuted, color: GP.Ink.muted)
            }
        }
        .padding(.horizontal, GP.Layout.screenInset)
        .padding(.vertical, 13)
    ...
    private var addButton: some View {
        Button { isAddingDrive = true } label: {
            PlusGlyph()
                ...
                .padding(19)
                .background(GP.Gradients.brandMark, in: Circle())
                .shadow(color: Color(hex: 0x5C3CDC, opacity: 0.5), radius: 16, y: 6)
        }
        .buttonStyle(.plain)
        .padding(.trailing, GP.Layout.screenInset)
        .padding(.bottom, 26)
    }
    ...
    private var inYear: [DriveRecord] {
        drives.filter { Calendar.gigPilot.component(.year, from: $0.start) == year }
    }
...
private struct TabSelector: View {
    ...
    var body: some View {
                ...
                Button {
                    ...
                    VStack(spacing: 9) {
                        Text(tab.label)
                            ...
                            .tracking(0.7)
                            .foregroundStyle(selection == tab ? GP.Ink.primary : GP.Ink.muted)
                        Rectangle()
                            .fill(selection == tab
        ...
        .overlay(alignment: .bottom) {
            Rectangle().fill(GP.Surface.dividerSoft).frame(height: 1)
        }
    }
}
