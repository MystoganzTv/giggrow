...
final class MileageTests: XCTestCase {
    ...
    func testSteppingMonthsDoesNotSkip() {
        ...
        selection.step(-1)
        XCTAssertEqual(Calendar.gigGrow.component(.month, from: selection.anchor), 2)
    }
    ...
    func testChangingRangeKeepsTheAnchor() {
        ...
        selection.use(.month)
        XCTAssertEqual(Calendar.gigGrow.component(.month, from: selection.anchor), 6)
    ...
    func testHeaderPutsAFutureLookingDateInLastYear() {
        ...
        let year = Calendar.gigGrow.component(.year, from: found ?? .distantFuture)
    ...
    func testDayNumberBeatsColumnPosition() {
        ...
        XCTAssertEqual(Calendar.gigGrow.component(.day, from: resolved ?? .distantPast), 3)
    }
    ...
    func testWeekdayNameUsedWhenNoNumberWasRead() {
        ...
        XCTAssertEqual(Calendar.gigGrow.component(.day, from: resolved ?? .distantPast), 4)
    }
    ...
    func testColumnIndexIsTheFallback() {
        ...
        XCTAssertEqual(Calendar.gigGrow.component(.day, from: resolved ?? .distantPast), 6)
    }
    ...
    func testWipeCoversEveryModelInTheSchema() {
        XCTAssertEqual(Seed.erasedTypeCount, GigGrowSchema.all.count,
                       "Seed.eraseEverything must delete every model in GigGrowSchema.all")
    }
    ...
    private func assertSameDay(_ actual: Date?, _ expected: Date,
        ...
        XCTAssertTrue(Calendar.gigGrow.isDate(actual, inSameDayAs: expected),
